...
namespace System.Linq.Tests
{
    public class MinMaxTests
    {
        ...
        public void NaNFirstDoubleWithSelector()
        {
            ...
            var nanThenMinusTen = new[] { -1F, -10, double.NaN, 10, 200, 1000 };
            var nanThenMinValue = new[] { double.MinValue, 3000F, 100, 200, double.NaN, 1000 };
            Assert.True(double.IsNaN(nanThenOne.Min(x => x)), "AAAAAA");
            Assert.True(double.IsNaN(nanThenMinusTen.Min(x => x)), "BBBBBBBB");
            Assert.True(double.IsNaN(nanThenMinValue.Min(x => x)), "CCCCCCCCC");
            Assert.False(double.IsNaN(nanThenOne.Max(x => x)), "yyyyy");
            Assert.False(double.IsNaN(nanThenMinusTen.Max(x => x)), "zzzzzz");
           Assert.False(double.IsNaN(nanThenMinValue.Max(x => x)), "wwwwwwwww");
            var nanWithNull = new[] { default(double?), double.NaN, default(double?) };
            Assert.True(double.IsNaN(nanWithNull.Min(x => x).Value), "DDDDDDDDD");
            Assert.True(double.IsNaN(nanWithNull.Max(x => x).Value), "EEEEEEEEEE");
        }
